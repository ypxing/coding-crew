...
description: Triage findings from the latest crew:afk code review report, challenge each one critically, implement sensible ones using TDD, commit touched files, and print a summary. Trigger with /address-code-review.
argument-hint: "Optional: path to report file"
---

# Address Code Review

You are working through the findings from an crew:afk code review report. Follow every step below in order.
