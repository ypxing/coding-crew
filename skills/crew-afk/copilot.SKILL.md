...
```bash
node "$(git rev-parse --show-toplevel)/.coding-crew/crew-afk/main.mjs" run --platform copilot "$@"
```
...
- `agent definition is not visible from a worktree` → Copilot resolves `--agent` from the
  worker's own directory, so commit `.github/agents/` or re-install user-level with
  `TARGET_REPO=$HOME`. `node .coding-crew/crew-afk/main.mjs doctor --platform copilot`
