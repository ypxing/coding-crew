...
description: >
  ...
  subagent (a separate `claude -p` process in an isolated git worktree), then housekeeping the
  result. Loops until no issues remain or all are stalled. Reviews every branch before it merges.
  Use when asked to run an AFK sprint or implement all open issues. Trigger with /crew-afk.
...
Every worker is its own `claude -p --agent crew-coder` child process, three at a time by
default (`--max-parallel`), each with its own worktree as cwd — not an `Agent` call in this
session, so this context stays empty and a hung worker times out instead of hanging the
sprint.
...
2. Poll the backgrounded run on a short interval and relay each new line as it arrives.
   ...
   entirely** — each dispatch is already visible live in its own herdr pane, so poll
   silently and only speak up when the run exits or hits something that needs the user's
   decision (e.g. a stale-branch blocker). The summary printed at the end is the
...
- Missing `crew-coder` / `crew-code-reviewer` definitions → the same re-install fixes it;
  `node "$CREW_AFK" doctor --platform claude` names what is absent.
