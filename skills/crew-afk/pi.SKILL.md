...
name: crew-afk
description: >
  Implements all ready-for-agent issues by dispatching each to a crew-coder subagent (a separate pi
...
Pass CLI-looking arguments straight through — `--model`, `--prd-audit`, `--fix-findings`,
`--max-parallel N`, `--jira TICKET-123`, a `.scratch/<feature-slug>/…` path — never rewrite
those. A bare word, typo, or free-form phrase is resolved first, below.
...
2. Poll and relay each new line; `[STEP]` lines and a throttled heartbeat go to
   **stderr**, so read that too. **If `ORCA_ENV=1`, skip polling** — this pane is prompted
   directly once the sprint finishes or stalls. Under `HERDR_ENV=1`, keep polling. The
