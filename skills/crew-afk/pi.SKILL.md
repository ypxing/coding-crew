...
```bash
node "$(git rev-parse --show-toplevel)/.coding-crew/crew-afk/main.mjs" run --platform pi "$@"
```
...
- `cannot find crew-afk's scripts/ dir` → the skill is half-installed; re-run
  `./install.sh pi --skill crew-afk`.
- Missing `crew-coder` / `crew-code-reviewer` definitions → the same re-install fixes it;
  `node .coding-crew/crew-afk/main.mjs doctor --platform pi` names what is absent.
