...
Every unselected `*.SKILL.md` is deleted from the destination after copy. `crew-afk` uses the first form for the platforms that still ship a prose orchestrator — with codex cut over, that is `copilot` alone resolving to `skills/crew-afk/dispatch.SKILL.md`, with its differences in `skills/crew-afk/fragments/copilot/<key>.md` (the mechanism stays until the last prose body goes, and `tests/shared-dispatch-body.bats` now asserts the platform list is non-empty so it cannot pass by iterating nothing). **pi and codex resolve the second form** (`skills/crew-afk/<platform>.SKILL.md`) because their orchestrator is a program, so each body is a launcher with nothing to vary. `scripts/render-skill.sh <skill> <platform>` renders a body by inlining `{{FRAGMENT:<key>}}` from that platform's fragment dir and substituting `{{PLATFORM}}`; `install.sh` runs it for every `SKILL.md` it installs. A missing fragment or a surviving `{{...}}` is a hard error — a body with a hole in it would ship an instruction gap to the model. `fragments/` is a build input and is never installed (and a `fragments/` tree left by an older install is pruned).
...
**pi and codex specifics: the orchestrator is a program, not a prompt.** `crew-afk` on pi ships a ~390-word launcher (`skills/crew-afk/pi.SKILL.md`) whose entire job is to run `node .coding-crew/crew-afk/main.mjs run --platform pi`, stream its output, and report its exit code (`0` finished · `2` stalled · `3` nothing ready · `1` setup error). The state machine — rounds, issue selection, worktrees, the `verify → review → merge → close` chain, promotion, stall detection, wrap-up — lives in `orchestrator/` (see `docs/crew-afk-orchestrator.md`). Workers are still separate `pi -p` processes launched through `skills/crew-afk/scripts/dispatch-agent.sh`, which resolves the agent definition (`.pi/agents/<name>.md`, then `~/.pi/agent/agents/<name>.md`), strips its frontmatter into `--append-system-prompt`, maps `tools:` onto `--tools`, and runs the worker with the issue's worktree as cwd — the program now calls it, with a hard per-worker timeout, instead of a prompt writing `&` and `wait`.

Why these platforms first: both were already subprocess-based, so each adapter was a script that already existed. Neither cutover is partial — `fragments/pi/` and `fragments/codex/` are **deleted**, not kept in parallel, because two maintained orchestrators is the drift this repo keeps paying for. The prose assertions that used to police those bodies now run against the code (`tests/orchestrator/*.test.mjs`, via `tests/orchestrator.bats`); `tests/helpers/render.bash` holds the two lists (`AFK_PROSE_VARIANTS`, `AFK_LAUNCHER_VARIANTS`) that say which platforms are prose and which are launchers, so the next cutover moves a name once, and `tests/crew-afk-launcher.bats` asserts every launcher against the same rules.

Running codex for real found two bugs that no amount of prose review would have: a worker could not commit (codex's `workspace-write` sandbox keeps `.git` read-only, and a linked worktree's index lives under the *main* repo's `.git`, so `git add` failed with `Operation not permitted` — `dispatch-codex-agent.sh` now names the worktree's git common dir as an explicit writable root), and the AC gate could never pass a criterion that ends "and the tests pass", because a read-only reviewer cannot run anything. The second is platform-general: the review prompt now states which checks `verify-worktree.sh` already ran in that worktree, and `agents/crew-code-reviewer/protocol.md` says a stated `pass` is the evidence for the execution half of a criterion, while `not_run` remains evidence of nothing.
...
**codex dispatch specifics:** Codex has native subagents, but they share the parent's working root, so `crew-afk` on codex dispatches each worker as a separate `codex exec` process through `skills/crew-afk/scripts/dispatch-codex-agent.sh` — called by `orchestrator/lib/dispatch.mjs`, with a hard per-worker timeout. That script resolves `.codex/agents/<name>.toml` (then `~/.codex/agents/<name>.toml`), prepends `developer_instructions` to the prompt (codex exec has no `--append-system-prompt`), maps `model`/`model_reasoning_effort`/`sandbox_mode` onto CLI flags, runs `codex exec --cd <worktree> --add-dir $MAIN_ROOT`, and captures the report via `--output-last-message`. Reports land in the same `.scratch/<slug>/dispatch/<slug>.report.md` path. **The sandbox is part of the adapter contract:** in `workspace-write` mode codex keeps `.git` read-only even inside a directory passed with `--add-dir`, and a linked worktree's index lives at `<main>/.git/worktrees/<name>/index.lock`, so the script passes `-c sandbox_workspace_write.writable_roots=["<git-common-dir>"]`. Without it a worker edits files it can never stage — observed as a whole sprint stalling at round 1 with every branch `blocked`. This path targets the **local `codex` CLI only** — it needs a local clone for worktrees and pre-existing auth. Hosted Codex (Codex in ChatGPT, Codex cloud/web) is out of scope: no persistent working root.
...
- `crew-afk` — orchestrator that spawns parallel `crew-coder` agents, merges completed branches, runs crew-code-reviewer, and loops until all ready-for-agent issues are done. Declares `agent-deps: [crew-coder, crew-code-reviewer]` — installing the skill pulls in both agents automatically. **On pi and codex the orchestrator is a program** (`orchestrator/` → `.coding-crew/crew-afk/`, launched by a <500-word `SKILL.md`); on claude/copilot it is still a prose body. Both drive the same bash mechanism layer, so the pipeline is identical either way. Skill-local scripts in `skills/crew-afk/scripts/`: `merge-branches.sh` (per-branch no-ff merge with conflict abort), `close-issue.sh` (Status rewrite + file move), `squash-commits.sh`, `session-init.sh` (also writes `sprint.env`, including the sprint's two policy flags — see below), `coverage-validation.sh` (opt-in — see below), `promote-findings.sh` (findings promotion — see below), `receipts.sh` (gate receipts — see below), `cleanup-worktrees.sh` (mechanical worktree/branch teardown — see below), `trace.sh` / `state.sh` / `crew-summary.sh` (sprint mechanism — see below), `verify-worktree.sh` (independently runs the project's typecheck, lint, and test checks in a worker's worktree before merge — merges are only permitted after this passes). Any check that fails exits non-zero. An undiscoverable check command is reported as `not_run` and listed as a coverage gap in the summary, never as passing; a missing test command is fatal (nothing was verified), while missing lint/typecheck are non-fatal so repos that legitimately have neither don't stall every sprint.. Pipeline order per branch: worker returns → schema pre-filter → verify-worktree → per-branch code review (acceptance criteria + findings) → merge → close, then squash once per sprint. Acceptance criteria are verified **before** the merge, by the reviewer — never by the coder that wrote the code, and no longer by a second pass over the same diff; a branch whose criteria are unmet, or whose review never completed, is demoted to `partial` and never merged. Cleanup deletes only merged branches, removing each merged worktree before its branch ref (a checked-out ref cannot be deleted); partial, verification-failed, criteria-unmet and review-not-run branches are retained with their branch name recorded under `retained_branches` in `sprint-state.json` so the next round resumes in place.
  ...
  **Sprint mechanism (not prose).** An orchestrator prompt is not a state machine, and the parts of it that were one behaved accordingly: an observed sprint re-derived the feature slug with `jq -r .feature_slug "$(ls -1 .scratch/*/sprint-state.json | head -n1)"` — an alphabetical-first glob — directly beneath a comment reading "Never re-derive it", logged `[DISPATCH]` twice because both the prompt and `dispatch-agent.sh` wrote it, and rendered its summary from lists it had been carrying in context since round 1. Four scripts now own that:
  ...
  Both orchestrator bodies are held under 3,000 words by `tests/crew-afk-state.bats`, which also asserts the behaviour the deleted prose used to describe; a budget is the only thing that stops mechanism creeping back in as prose. **On a launcher platform (pi, codex) the budget is 500 words**, because the body is a launcher and the mechanism is `orchestrator/` — see `tests/crew-afk-launcher.bats`, which runs every rule over `AFK_LAUNCHER_VARIANTS` and fails if a launcher starts naming `state.sh`, `receipts.sh`, `merge-branches.sh`, `close-issue.sh`, `promote-findings.sh`, `verify-worktree.sh` or `git worktree add` again. A launcher that re-describes the pipeline is a second orchestrator.
