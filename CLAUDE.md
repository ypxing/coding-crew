...
- `scripts/tracker/mark-issue-done.sh` → `.coding-crew/scripts/mark-issue-done.sh` — the `mark-done` operation. It refuses to close an issue in two cases, and both used to be prose that nothing enforced. Exit **3** when an orchestrator owns the close (`.scratch/<feature-slug>/.orchestrated` exists, written by crew-afk's `session-init.sh` and removed by `crew-summary.sh` at sprint end — or `CREW_ORCHESTRATED=1` is set by the dispatchers); exit **4** when a `- [ ]` remains under `## Acceptance criteria` or `## Cross-cutting Requirements`. `--force` overrides both, for a marker left by a crashed sprint or a criterion recorded as descoped.
  ...
  **One writer per issue file.** The close was mechanical but the *check-off* was not, so the refusal was half a rule: exit 4 is "refuse while a `- [ ]` remains under `## Acceptance criteria` or `## Cross-cutting Requirements`", a gate a worker satisfies by ticking its own boxes — self-attestation of the one check that is deliberately the reviewer's. `solve-issue` §7/§8 therefore branch on the *same fact* `mark-done` checks (`CREW_ORCHESTRATED=1`, or a `.scratch/*/.orchestrated` marker) rather than on who the caller says it is: orchestrated ⇒ **no issue-file writes at all** — no tick, no `Status:` rewrite, no move, and no `## Unmet criteria` section — the criteria are reported in the structured result and nothing else. Unorchestrated, the direct `/solve-issue` path is unchanged: it ticks its own boxes and runs `mark-done`, because it is the only writer there. The tick moved to the mechanism that already owned the `Status:` rewrite: `close-issue.sh` ticks every remaining `- [ ]` under both sections, in the same portable single-pass awk rewrite, and only after its receipt gate passes — a refused close leaves the file exactly as the worker left it. Section scoping matches `mark-issue-done.sh`'s guard exactly, so the two agree on which boxes are criteria and a `- [ ]` in `## Notes` is left alone. `crew-coder`'s § "Issue Ownership" is consequently one line pointing at `solve-issue` §7: it no longer restates an enforcement its own gate performs, and no longer contradicts the procedure it invokes. Guarded by `tests/worker-close-guard.bats` and `tests/crew-afk-scripts.bats`.
