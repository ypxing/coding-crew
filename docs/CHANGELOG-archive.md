...
Older entries moved out of `CHANGELOG.md` to keep that file a manageable size to read and edit
(it had grown to ~120KB / 1,026 lines — most of it history nobody needed for the next release).
This file is not read by tooling: `.github/workflows/release.yml` and `scripts/cut-release.sh`
only ever read `CHANGELOG.md`'s own top entry. See `CHANGELOG.md` for v1.28.0 onward.

The `## [Unreleased]` heading below is not a mistake to "fix": before v1.27.0 this repo tagged
releases in batches rather than one tag per commit, and this section is the accumulated entry for
several changes that shipped together as v1.24.0–v1.26.0 without ever being split into per-version
headings. Left as originally written rather than retroactively re-labelled.
