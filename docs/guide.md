...
| Sprint stalls with a branch retained `merge-conflict` | Two verified branches touch overlapping code. crew-afk's retry already gave the coder the conflicted merge to resolve, and it didn't land within the retry cap | `git checkout <feature-branch> && git merge --no-ff <branch>`, resolve by hand, commit, then re-run `/crew-afk` — it sees the branch as already merged and closes the issue |
